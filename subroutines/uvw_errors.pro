Pro uvw_errors, ra, dec, pmra, pmdec, vrad, dist, e_ra, e_dec, e_pmra, e_pmdec, e_vrad, e_dist, $
                ...
                omega_u=omega_u, omega_v=omega_v, omega_w=omega_w
  ;uvw_errors, ra, dec, pmra, pmdec, rv, dist, 0., 0., epmra, epmdec, erv, edist, U, V, W, EU, EV, EW
  ;+
  ; NAME:
  ;       UVW_ERRORS
  ;
  ...
  ;U = T1 * vrad + (T2*pmra+T3*pmdec)*k/plx
  ;U = M0 * vrad + N0/plx
  ;M0 = T1
  ;N0 = (T2*pmra+T3*pmdec)*k
  ;dN0/dpmra = T2*k = k*( -A[0,0]*sina      + A[0,1]*cosa)
  
  ;U/T1 = vrad + T2/T1*k*pmra/plx + T3/T1*k*pmdec/plx
  ;V/T4 = vrad + T5/T4*k*pmra/plx + T6/T4*k*pmdec/plx
  ...
  omega_v = T4*vrad
  omega_w = T7*vrad
  ;U = ((-0.0548755604*cos(ra*0.0174533)*cos(dec*0.0174533)-0.8734370902*sin(ra*0.0174533)*cos(dec*0.0174533)-0.4838350155*sin(dec*0.0174533))*radial_velocity+(0.0548755604*sin(ra*0.0174533)-0.8734370902*cos(ra*0.0174533))*4.743717361*PMRA/PARALLAX + (0.0548755604*cos(ra*0.0174533)*sin(dec*0.0174533)+0.8734370902*sin(ra*0.0174533)*sin(dec*0.0174533)-0.4838350155*cos(dec*0.0174533))*4.743717361*pmdec/parallax)
  ;V = ((0.4941094279*COS(RA*0.0174533)*COS(DEC*0.0174533)-0.4448296300*SIN(RA*0.0174533)*COS(DEC*0.0174533)+0.7469822445*SIN(DEC*0.0174533))*radial_velocity+(-0.4941094279*SIN(RA*0.0174533)-0.4448296300*COS(RA*0.0174533))*4.743717361*PMRA/PARALLAX + (-0.4941094279*COS(RA*0.0174533)*SIN(DEC*0.0174533)+0.4448296300*SIN(RA*0.0174533)*SIN(DEC*0.0174533)+0.7469822445*COS(DEC*0.0174533))*4.743717361*PMDEC/PARALLAX)
  ;W = ((-0.8676661490*COS(RA*0.0174533)*COS(DEC*0.0174533)-0.1980763734*SIN(RA*0.0174533)*COS(DEC*0.0174533)+0.4559837762*SIN(DEC*0.0174533))*radial_velocity+(0.8676661490*SIN(RA*0.0174533)-0.1980763734*COS(RA*0.0174533))*4.743717361*PMRA/PARALLAX + (0.8676661490*COS(RA*0.0174533)*SIN(DEC*0.0174533)+0.1980763734*SIN(RA*0.0174533)*SIN(DEC*0.0174533)+0.4559837762*COS(DEC*0.0174533))*4.743717361*PMDEC/PARALLAX)
  ;VTOT = SQRT(POWER(((-0.0548755604*COS(RA*0.0174533)*COS(DEC*0.0174533)-0.8734370902*SIN(RA*0.0174533)*COS(DEC*0.0174533)-0.4838350155*SIN(DEC*0.0174533))*radial_velocity+(0.0548755604*SIN(RA*0.0174533)-0.8734370902*COS(RA*0.0174533))*4.743717361*PMRA/PARALLAX + (0.0548755604*COS(RA*0.0174533)*SIN(DEC*0.0174533)+0.8734370902*SIN(RA*0.0174533)*SIN(DEC*0.0174533)-0.4838350155*COS(DEC*0.0174533))*4.743717361*PMDEC/PARALLAX),2)+POWER(((0.4941094279*COS(RA*0.0174533)*COS(DEC*0.0174533)-0.4448296300*SIN(RA*0.0174533)*COS(DEC*0.0174533)+0.7469822445*SIN(DEC*0.0174533))*radial_velocity+(-0.4941094279*SIN(RA*0.0174533)-0.4448296300*COS(RA*0.0174533))*4.743717361*PMRA/PARALLAX + (-0.4941094279*COS(RA*0.0174533)*SIN(DEC*0.0174533)+0.4448296300*SIN(RA*0.0174533)*SIN(DEC*0.0174533)+0.7469822445*COS(DEC*0.0174533))*4.743717361*PMDEC/PARALLAX),2)+POWER(((-0.8676661490*COS(RA*0.0174533)*COS(DEC*0.0174533)-0.1980763734*SIN(RA*0.0174533)*COS(DEC*0.0174533)+0.4559837762*SIN(DEC*0.0174533))*radial_velocity+(0.8676661490*SIN(RA*0.0174533)-0.1980763734*COS(RA*0.0174533))*4.743717361*PMRA/PARALLAX + (0.8676661490*COS(RA*0.0174533)*SIN(DEC*0.0174533)+0.1980763734*SIN(RA*0.0174533)*SIN(DEC*0.0174533)+0.4559837762*COS(DEC*0.0174533))*4.743717361*PMDEC/PARALLAX),2))
  ;DELTA = (U-U0)^2+(V-V0)^2+(W-W0)^2; d(DELTA)/d(RV) = 2*(U-U0)*d(U)/d(RV) + 2*(V-V0)*d(V)/d(RV) + 2*(W-W0)*d(W)/d(RV) = 2*(U-U0)*T1 + 2*(V-V0)*T4 + 2*(W-W0)*T7 = 2*T1*(T1*RV + T2*k*pmra/plx + T3*k*pmdec/plx - U0) + 2*T4*(T4*RV + T5*k*pmra/plx + T6*k*pmdec/plx - V0) + 2*T7*(T7*RV + T8*k*pmra/plx + T9*k*pmdec/plx - W0); d(DELTA)/d(RV) = 0 => 0 = T1*(T1*RV + T2*k*pmra/plx + T3*k*pmdec/plx - U0) + T4*(T4*RV + T5*k*pmra/plx + T6*k*pmdec/plx - V0) + T7*(T7*RV + T8*k*pmra/plx + T9*k*pmdec/plx - W0) = T1^2*RV + T4^2*RV + T7^2*RV + T1*(T2*k*pmra/plx + T3*k*pmdec/plx - U0) + T4*(T5*k*pmra/plx + T6*k*pmdec/plx - V0) + T7*(T8*k*pmra/plx + T9*k*pmdec/plx - W0); RV = -[T1*(T2*k*pmra/plx + T3*k*pmdec/plx - U0) + T4*(T5*k*pmra/plx + T6*k*pmdec/plx - V0) + T7*(T8*k*pmra/plx + T9*k*pmdec/plx - W0)]/(T1^2+T4^2+T7^2)
  ...
  if n_elements(T1) eq 1L then begin
    ...
    transformation_matrix = transpose([[[t1],[t2],[t3]],[[t4],[t5],[t6]],[[t7],[t8],[t9]]],[1,2,0])
    transformation_vector = transpose([[vrad], [k*pmra/plx], [k*pmdec/plx]])
  endelse
